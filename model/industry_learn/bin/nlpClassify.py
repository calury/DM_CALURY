...
def digitize_Cat(df_Base):
    ...
    df_Base2['id'] = df_Base2['id'].where(df_Base2['id'].notnull(), "")
    return df_Base2, Cat
    pass
...
def TFtesttfidf(cor, vectorizer, transformer):
    ...
    if vectorizer.transform(cor).shape[0] < 1:
        ...
        test = transformer.transform(
            vectorizer.transform(cor))  # 第一个fit_transform是计算tf-idf，第二个fit_transform是将文本转为词频矩阵  
        # word = vectorizer.get_feature_names()  # 获取词袋模型中的所有词语  
        weight = test.toarray()  # 将tf-idf矩阵抽取出来，元素a[i][j]表示j词在i类文本中的tf-idf权重 
    weight = np.nan_to_num(weight)  # todo 避免infinite inf 以及 too large number
...
def TFtraintfidf(cor):
    '''获取词语集合，以及权重'''
    vectorizer = CountVectorizer(max_features=2000)  # TODO 该类会将文本中的词语转换为词频矩阵，矩阵元素a[i][j] 表示j词在i类文本下的词频  
    transformer = TfidfTransformer()  # 该类会统计每个词语的tf-idf权值  
    train = transformer.fit_transform(
        vectorizer.fit_transform(cor))  # 第一个fit_transform是计算tf-idf，第二个fit_transform是将文本转为词频矩阵  
    word = vectorizer.get_feature_names()  # 获取词袋模型中的所有词语  
    print('Size of feature is', len(word))
    print(type(vectorizer.fit_transform(cor)), "+++++++++++++++++++++")
    print(type(train), "------------------------------------------------")
...
def SplitTrainning(df_Base, size_Train, df_Predict, size_Predict, Cat, repeatNo):
    ...
    for j in range(repeatNo):
        ...
        logger.info("training the %s classify" % j)
        tic = time.clock()
        toc = time.clock()
        Y = []
        ...
        logger.info("training the %s classify finished" % j)
        ...
        for i in range(batch_Predict):
            ...
            weighttest = TFtesttfidf(xtestcor, vector, transformer)
            logger.info("testing the %s classify strated" % i)
            ypredict = clff.predict(weighttest.tolist())
            Y = Y + ypredict.tolist()
            logger.info("testing the %s classify finished" % i)
...
def TrainByIPC(df_Base, size_Train, df_Predict, size_Predict, Cat, repeatNo, industryIPC):
    logger.info("train size: %d, predict size: %d, classify numbers: %d" % (size_Train, size_Predict, repeatNo))
    ...
    for i in IPCs:
        if i in industryIPC:
            ...
            if size_Predict_temp > 0 and size_Train_temp > 10:
                print('Now,Processing IP in', i, 'with train size', df_Base_temp.shape[0], 'and predict size',
                      df_Predict_temp.shape[0])
                logger.info('Processing %s , with training size %d, and predict size %d' % (
                    i, df_Base_temp.shape[0], df_Predict_temp.shape[0]))
                for times in range(10):
                    try:
                        df_Temp = SplitTrainning(df_Base_temp, size_Train_temp, df_Predict_temp, size_Predict_temp, Cat,
                                                 repeatNo)
                    except:
                        logger.error("error  in %s, try %d" % (i, times))
                    else:
                        df_T = pd.concat([df_T, df_Temp], axis=0, ignore_index=False)
                        logger.info("success in %s, try %d" % (i, times))
                        break
...
if __name__ == "__main__":
    ...
    for i in name:
        ...
        logger.info("loading %d file, and url %s" % (j, name))
    # 3.train the data
    print('-------------Start Training Data--------------')
    df_Base, Cat = digitize_Cat(df_Base)

    df = TrainByIPC(df_Base, 400, df_Test, 400, Cat, 2, industryIPC)
